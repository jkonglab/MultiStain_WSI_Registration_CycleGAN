...
class DIRNet(object):
  ...
  def deploy(self, dir_path, x, y, name1):
    z,v, im_flat, Ia, Ib, Ic, Id = self.sess.run([self.z, self.v, self.im_flat, self.Ia, self.Ib, self.Ic, self.Id], {self.x:x, self.y:y})
    ...
    print("final warped image:{}".format(z.shape))
    #print("dvf:{}".format(v))
