...
def dk_s1(input, k, reuse=False, norm='instance', is_training=True, name=None):
	with tf.variable_scope(name, reuse=reuse):
...
def dk_pool(input,reuse=False,is_training=True, name=None):
	with tf.variable_scope(name, reuse=reuse):
		output = tf.nn.max_pool2d(input, [1,2,2,1], [1,2,2,1], name=None)
		return output
